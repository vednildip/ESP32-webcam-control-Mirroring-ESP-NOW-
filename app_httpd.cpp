...
#define CHUNK_LENGTH 246
#define WIFI_SSID "vivo_office"
const uint8_t receiver_mac[] = { 0x24, 0x58, 0x7C, 0xD3, 0x5F, 0x9C };
bool espNowInitialized = false;
bool connected = false;
volatile bool ack_received = false;
//volatile bool sending = false;
volatile bool sendingEspNow = false;
volatile bool sendingWeb    = false;
volatile bool streamActive = false;
extern bool espnowActive;
unsigned long espnowStartTime = 0;
...
void onDataSent(const wifi_tx_info_t* tx_info, esp_now_send_status_t status) {
  ...
  sendingEspNow = false;   // ⭐ REQUIRED
}
// --- Chunked Data Sender ---
void sendPacketData(const char* buf, uint16_t len, uint16_t chunkLength) {
  uint8_t buffer[chunkLength];
  size_t fullChunks = len / chunkLength;
  size_t remainder = len % chunkLength;

  Serial.println("📤 Starting packet transmission");

  for (size_t i = 0; i < fullChunks; ++i) {
    memcpy(buffer, buf + (i * chunkLength), chunkLength);
    ack_received = false;

    if (esp_now_send(receiver_mac, buffer, chunkLength) != ESP_OK) {
      Serial.println("❌ Send failed");
      break;
    }

    unsigned long start = millis();
    while (!ack_received && millis() - start < 1000) delay(1);

    if (!ack_received) {
      Serial.println("⚠️ No ACK received, stopping");
      break;
    }
  }

  if (remainder) {
    memcpy(buffer, buf + (len - remainder), remainder);
    ack_received = false;

    if (esp_now_send(receiver_mac, buffer, remainder) == ESP_OK) {
      unsigned long start = millis();
      while (!ack_received && millis() - start < 1000) delay(1);
    }
  }
...
void captureAndSendFrame() {
  if (!connected || sendingEspNow || sendingWeb) return;
  ...
  if (fb->format == PIXFORMAT_JPEG) {
    ...
    Serial.println("⚠️ Non-JPEG format not supported");
  }
...
static unsigned long lastEspNow = 0;
  while (true) {
   ...
   /*   if (!connected || sending) return ESP_OK;
 if (millis() - lastEspNow > 150) { // 150ms interval
    captureAndSendFrame();
    lastEspNow = millis();
}*/
    fb = esp_camera_fb_get();
...
if (connected && !sendingEspNow &&
    millis() - lastEspNow > 150 &&
    _jpg_buf && _jpg_buf_len < 60000) {

  lastEspNow = millis();
  sendingEspNow = true;
  sendPacketData((const char*)_jpg_buf, _jpg_buf_len, CHUNK_LENGTH);
}
